...
COPY index.js banner.js parseBool.js validateConfig.js validateTraefikConfig.js ./
...
VALIDATECONFIG_JS_EOF
    ...
    # Create validateTraefikConfig.js (embedded)
    cat > "$work_dir/validateTraefikConfig.js" << 'VALIDATETRAEFIKCONFIG_JS_EOF'
'use strict';
...
function findLatestBackup(configPath) {
  ...
  const backups = files.filter(function(f) { return pattern.test(f); }).sort();
...
function validateTraefikConfig(traefikDir, log) {
  ...
  var backupPath = buildBackupPath(configPath);
  try {
  ...
  var commented = commentOutProviders(content);
  var newContent = commented + '\n' + buildProvidersBlock() + '\n';
...
function revertTraefikConfig(traefikDir, log) {
  ...
  var configPath = findTraefikConfigFile(traefikDir);
  if (!configPath) {
...
module.exports = {
  validateTraefikConfig: validateTraefikConfig,
  revertTraefikConfig: revertTraefikConfig,
  buildBackupPath: buildBackupPath,
  findTraefikConfigFile: findTraefikConfigFile,
  hasExpectedProviders: hasExpectedProviders,
  commentOutProviders: commentOutProviders,
  buildProvidersBlock: buildProvidersBlock,
  isModifiedByTool: isModifiedByTool,
  findLatestBackup: findLatestBackup,
  yamlGetValue: yamlGetValue,
  CONFIG_MARKER: CONFIG_MARKER,
};
VALIDATETRAEFIKCONFIG_JS_EOF
